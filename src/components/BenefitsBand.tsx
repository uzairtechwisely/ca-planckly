...
type BenefitIcon = "shield-check" | "zap" | "sparkles";

function Icon({ icon }: { icon: BenefitIcon }) {
...
export function BenefitsBand({
  ...
  eyebrow: string;
  heading: string;
  items: Array<{ icon: BenefitIcon; title: string; description: string }>;
}) {
  return (
    <section id="benefits" className="relative mx-auto w-full max-w-6xl px-4 py-16 sm:py-20">
      ...
      <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item, idx) => (
          <motion.div
            ...
            className="rounded-[28px] border border-[rgba(0,0,0,0.08)] bg-white/70 p-6 shadow-sm backdrop-blur transition hover:bg-white"
          >
